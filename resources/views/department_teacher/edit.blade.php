...
<div class="max-w-xl">
    ...
    <div class="card">
        <div class="px-6 py-5 border-b border-gray-50">
            <h1 class="text-base font-semibold text-gray-900">Edit Assignment</h1>
            <p class="text-sm text-gray-400 mt-0.5">Update this teacher–department assignment.</p>
        </div>
        ...
        <div class="p-6">
            ...
            <form action="{{ route('department_teacher.update', $row->id) }}" method="POST" class="space-y-5">
                @csrf @method('PUT')
                ...
                <div>
                    ...
                    <select id="teacher_id" name="teacher_id" required
                        ...
                        @foreach ($teachers as $teacher)
                            <option value="{{ $teacher->id }}" {{ $row->teacher_id == $teacher->id ? 'selected' : '' }}>
                                {{ $teacher->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="department_id" name="department_id" required
                        ...
                        @foreach ($departments as $department)
                            <option value="{{ $department->id }}" {{ $row->department_id == $department->id ? 'selected' : '' }}>
                                {{ $department->name }}
                            </option>
                        @endforeach
                    </select>
